...
const siteBase = location.hostname.endsWith('github.io') ? '/grand-royal-dine/' : '/';
function sitePath(path='') { return siteBase + path; }
function setSitePaths() {
  document.querySelectorAll('[data-nav]').forEach(a => a.href = sitePath(a.dataset.nav || ''));
  document.querySelectorAll('[data-home]').forEach(a => a.href = sitePath(''));
  document.querySelectorAll('[data-logo]').forEach(img => img.src = sitePath('assets/logo.png'));
}
...
function inject() {
  ...
  document.querySelectorAll('[data-menu-link]').forEach(a => a.href = sitePath('assets/menu-2.pdf'));
}
